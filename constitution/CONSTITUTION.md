...
この文書は、Looker の開発組織における最上位 contract です。
...
Looker の開発組織の目的は current procedure への服従ではありません。
...
Looker is a filesystem-mutating Rust CLI. Dry-run plans, applied filesystem effects, produced shortcuts/directories, and source candidate identity must not be conflated.

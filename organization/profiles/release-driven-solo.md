...
Looker の current release-driven development で使用する Operating Model を定義します。
...
- Cargo.toml / Cargo.lock remain Rust dependency authority; mise does not add a competing Rust pin.
- dry-run output is planning evidence, not proof that filesystem mutation succeeded.
- mutation tests use disposable fixtures/sandboxes and never unrelated user directories.
- release binary evidence is bound to source SHA / target platform / artifact identity.
